...
public class Partida {
	...
	private boolean isNotificadoFin;
	...
	public void reset(ReglasJuego reglas) {
		...
		for (int i=0; i<observers.size(); i++) {
			observers.elementAt(i).onReset(tablero, turno);
			observers.elementAt(i).onUndoNotPossible();
		}
	}
	...
	public boolean undo() {
		...
		if (res) {
			...
			for (int i=0; i<observers.size(); i++) {
				observers.elementAt(i).onUndo(tablero, turno, canUndo());
			}
		}
		return res;
	}
	...
	public boolean ejecutaMovimiento(Movimiento mov) throws MovimientoInvalido{
		...
		if (res) {
			...
			for (int i=0; i<observers.size(); i++) {
				observers.elementAt(i).onMovimientoEnd(tablero, turno, turno);
			}
			if (isTerminada()){
				if (!isNotificadoFin) {
					isNotificadoFin = true;
					for (int i=0; i<observers.size(); i++) {
						observers.elementAt(i).onPartidaTerminada(tablero, getGanador());
					}
				}
			}
		}
